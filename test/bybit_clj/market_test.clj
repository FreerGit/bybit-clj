...
(deftest get-time
  (testing "Get time REST"
    (d/chain (market/get-time)
             request-is-ok)))

(deftest get-kline
  (testing "Get kline REST"
    (let [body
          @(market/get-kline "inverse" "BTCUSDT" 60 {:limit 20})
          body-without-opt
          @(market/get-kline "inverse" "BTCUSDT" 60 {})]
      (request-is-ok body)
...
(deftest get-mark-price-kline
  (testing "Get mark kline REST"
    (let [body
          @(market/get-mark-price-kline "inverse" "BTCUSDT" 60 {:limit 20})
          body-without-opt
          @(market/get-mark-price-kline "inverse" "BTCUSDT" 60 {})]
      (request-is-ok body)
      (request-is-ok body-without-opt)
      (result-list-has-count body 20))))
...
(deftest get-index-price-kline
  (testing "Get index kline REST"
    (let [body
          @(market/get-index-price-kline "inverse" "BTCUSDT" 60 {:limit 20})
          body-without-opt
          @(market/get-index-price-kline "inverse" "BTCUSDT" 60 {})]
      (request-is-ok body)
      (request-is-ok body-without-opt)
      (result-list-has-count body 20))))
...
(deftest get-premium-index-price-kline
  (testing "Get premium kline REST"
    (let [body
          @(market/get-premium-index-price-kline "inverse" "BTCUSDT" 60 {:limit 20})
          body-without-opt
          @(market/get-premium-index-price-kline "inverse" "BTCUSDT" 60 {})]
      (request-is-ok body)
      (request-is-ok body-without-opt)
      (result-list-has-count body 20))))
...
(deftest get-instruments-info
  (testing "Get premium kline REST"
    (let [body
          @(market/get-instruments-info "inverse" {:symbol "BTCUSDT" :limit 20})]
      (request-is-ok body)
...
(deftest get-orderbook
  (testing "Get orderbook REST"
    (let [body
          @(market/get-orderbook "inverse" "BTCUSDT" {:limit 20})
          body-200
          @(market/get-orderbook "inverse" "BTCUSDT" {})]
      (request-is-ok body)
...
(deftest get-tickers
  (testing "Get tickers REST"
    (let [body
          @(market/get-tickers "spot" "BTCUSDT" {})]
      (request-is-ok body)
      (result-list-has-count body 1))))
...
(deftest get-funding-history
  (testing "Get funding history REST"
    (let [body
          @(market/get-funding-history "linear" "ETHPERP" {:limit 1})]
      (request-is-ok body)
      (result-list-has-count body 1))))
...
(deftest get-recent-trades
  (testing "Get recent trades REST"
    (let [body
          @(market/get-recent-trades "linear" "BTCUSDT" {:limit 1})]
      (request-is-ok body)
      (result-list-has-count body 1))))
...
(deftest get-open-interest
  (testing "Get open interest REST"
    (let [body
          @(market/get-open-interest "linear" "BTCUSDT" "1d" {:limit 50})]
      (request-is-ok body)
...
(deftest get-risk-limit
  (testing "Get risk limit REST"
    (let [body
          @(market/get-risk-limit "linear" "BTCUSDT")
          body-all
          @(market/get-risk-limit "linear")]
      (request-is-ok body)
...
(deftest get-delivery-price
  (testing "Get delivery price REST"
    (let [body
          @(market/get-delivery-price "linear" {})
          body-option
          @(market/get-delivery-price "option" {})]
      (request-is-ok body)
...
(deftest get-long-short-ratio
  (testing "Get long short ratio REST"
    (let [body
          @(market/get-long-short-ratio "linear" "BTCUSDT" "4h" 25)
          body-default
          @(market/get-long-short-ratio "inverse" "BTCUSD" "1h")]
      (request-is-ok body)
